...
export const Sidebar: React.FC = () => {
  ...
  const sections = [
    {
      title: t('overview'),
      items: [
        { icon: LayoutDashboard, label: isGlobalView ? t('globalDashboard') : t('regionalDashboard'), path: '/' },
      ]
    },
    {
      title: t('management'),
    ...
    //   title: t('sandboxes'),
    //   items: [
    //     { icon: Zap, label: t('merchantPortal'), path: '/portal' },
    //   ]
    // }
  ];
  ...
  return (
    <aside
      ...
      style={{ width: sidebarCollapsed ? '5rem' : '17.5rem' }}
    >
      <div className={`p-6 flex items-center gap-3 ${sidebarCollapsed ? 'justify-center' : 'justify-center md:justify-start'}`}>
        <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-lg shadow-red-500/20 flex-shrink-0">
          <img
            src="/Redtab_no_margin_logo.png"
            alt="Redtab"
      ...
      <nav className="flex-1 space-y-8 pb-10 mt-4">
        {sections.map((section, idx) => (
          <div key={idx} className={sidebarCollapsed ? 'space-y-2' : 'space-y-2 md:space-y-1'}>
            {!sidebarCollapsed && (
            ...
            {section.items.map((item) => (
              <div key={item.path} className="relative">
                <Link
                  ...
                  } ${sidebarCollapsed ? 'justify-center px-2' : 'justify-center md:justify-start px-2 md:px-8'}`}
                  title={sidebarCollapsed ? item.label : undefined}
                >
                  <item.icon size={18} strokeWidth={isActive(item.path) ? 2.5 : 2} className="flex-shrink-0" />
                  {!sidebarCollapsed && <span className="hidden md:inline text-sm">{item.label}</span>}
                </Link>

                {/* Tooltip - Show on mobile and when sidebar is collapsed on desktop */}
                ...
                {sidebarCollapsed && (
                  <div className="hidden md:block absolute left-full ml-2 top-1/2 -translate-y-1/2 px-3 py-2 bg-gray-900 text-white text-xs font-semibold rounded-lg whitespace-nowrap z-50 opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-opacity duration-200 pointer-events-none">
                    {item.label}
                    <div className="absolute right-full top-1/2 -translate-y-1/2 border-4 border-transparent border-r-gray-900" />
                  </div>
                )}
      ...
      <div className={`p-4 mt-auto border-t border-gray-50 ${sidebarCollapsed ? 'flex justify-center' : 'flex justify-center md:block'}`}>
        <div className={`bg-gray-50 rounded-2xl p-4 border border-gray-100 ${sidebarCollapsed ? 'flex items-center justify-center w-12 h-12' : 'flex items-center justify-center w-12 h-12 md:w-auto md:h-auto md:block'}`}>
          {!sidebarCollapsed ? (
            <>
              <p className="hidden md:flex text-2xs font-black text-gray-400 uppercase tracking-widest mb-2 items-center gap-2">
                <ClockIcon className="w-2.5 h-2.5" /> {isGlobalView ? t('globalSync') : t('regionalLock')}
              </p>
              <div className="hidden md:flex items-center gap-2 text-xs text-gray-600 font-medium">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                {isGlobalView ? t('allBridgesActive') : t('instanceLabel', { name: selectedSegment?.name })}
              </div>
              <div className="md:hidden w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" title={isGlobalView ? t('allBridgesActive') : t('instanceLabel', { name: selectedSegment?.name })} />
            </>
          ) : (
            <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" title={isGlobalView ? t('allBridgesActive') : t('instanceLabel', { name: selectedSegment?.name })} />
          )}
        </div>
      </div>
    </aside>
